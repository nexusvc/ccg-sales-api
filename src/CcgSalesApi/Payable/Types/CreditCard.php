...
class CreditCard extends Payable {
    ...
    public static function isValidCard($account, $cvc, $expiration) {

        if(!(bool) self::validCreditCard($account)['valid']) return false;
        
        if(!(bool) self::validCvc($cvc, self::validCreditCard($account)['type'])) return false;

        if(!is_array($expiration) || 
            is_array($expiration) && !array_key_exists('month', $expiration) || 
            is_array($expiration) && !array_key_exists('year', $expiration)) {
            return false;
        }

        if(!(bool) self::validDate($expiration['year'], $expiration['month'])) return false;
