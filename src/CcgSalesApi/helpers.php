...
if (! function_exists('formatState')) {
    function formatState($state, $style = 'abbr') {
        ...
        // Default to ABBR
        if(!array_key_exists($style, $states)) $style = 'abbr';
        // Return State on no Match
        if(!array_key_exists(strtoupper($state), $states[$style])) return $state;

        return $states[$style][ strtoupper($state) ];
    }
}
...
if(!function_exists('strip_country_prefix')) {
    function strip_country_prefix($phone) {
        $prefix = '1';
